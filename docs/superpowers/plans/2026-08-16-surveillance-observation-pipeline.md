...
- Migration head is `0033`. Any new migration chains from it.
...
| File | Responsibility |
|---|---|
| `services/biometric/occlusion.py` (create) | Landmark-visibility → occlusion stratum. The missing input the calibration design assumes. |
...
**Why this is first:** `FUSION_FLOORS` prices face down under occlusion and the design calls for a separate `ImpostorStats` per stratum, but **nothing in the codebase computes the stratum**. Without it, per-occlusion calibration cannot be selected at query time and a single pooled calibration silently returns a threshold that is too low for veiled faces — the exact failure the design exists to prevent.
...
```bash
git add services/biometric/occlusion.py tests/unit/test_occlusion.py
git commit -m "feat(biometric): occlusion stratum from landmark visibility
...
### Task 2: Per-modality gallery size in `to_readings`
...
**Interfaces:**
- Consumes: `fusion.FusedIdentity` (Task 0, existing), `rf_mapping.PositionFix` (existing), `occlusion.OcclusionStratum` (Task 1).
...
### Task 4: Ingest and query endpoints
...
**Interfaces:**
- Consumes: `build_observation`/`record` (Task 3), `rf_mapping.locate`, `occlusion.classify_occlusion` (Task 1).
