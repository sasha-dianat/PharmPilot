...
SYSTEM = (
    "Answer the pharmacist's question using ONLY the numbered sources provided. "
    "Cite the source_id in square brackets after each claim, e.g. [S3]. "
    "If the sources do not contain the answer, say you cannot answer from the available sources. "
    "Do not use outside knowledge. State uncertainty."
)


async def synthesize(question: str, chunks: list[object]) -> str | None:
    if not chunks:
        return None
    try:
...
def _prompt(question: str, chunks: list[object]) -> str:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        content = (_attr(chunk, "content", "") or "").strip()
